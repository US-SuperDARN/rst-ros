...
char progid[80]={"widescan 2025/09/24"};
char progname[256];
...
int rst_opterr(char *txt) {
  ...
  fprintf(stderr,"Please try: widescan --help\n");
  return(-1);
}
...
int main(int argc,char *argv[]) {
  ...
  if (rxonly) {
    strcpy(progid, "rxonlywidescan; BISTATIC");
    strcpy(progname, "rxonlywidescan; BISTATIC");
    txpow = 1;
  } else {
    if (fast) sprintf(progname,"widescan (fast)");
    else sprintf(progname,"widescan");
  }
  ...
  float *pwr0_arr = calloc(nBeams_per_scan*MAX_RANGE, sizeof(float));
  float *acfd_arr = calloc(nBeams_per_scan*MAX_RANGE*LAG_SIZE*2, sizeof(float));
  float *xcfd_arr = calloc(nBeams_per_scan*MAX_RANGE*LAG_SIZE*2, sizeof(float));
...
/*  total_scan_usecs = (scnsc-3)*1E6+scnus; */
  ...
  do {
    ...
    do {  /* start of scan */
      ...
      sprintf(logtxt,"Integrating period:%d intt:%ds.%dus (%02d:%02d:%02d:%06d)",
              iBeam,intsc,intus,hr,mt,sc,us);
      ...
      nave = SiteWideIntegrate(seq->lags, pwr0_arr, acfd_arr, xcfd_arr);
      if (nave < 0) {
      ...
      for (i=0; i < nBeams_per_scan; i++) {
        bmnum = scan_beam_number_list[i];
        memcpy(pwr0, pwr0_arr+i*sizeof(float)*MAX_RANGE, sizeof(float)*MAX_RANGE);
        memcpy(acfd, acfd_arr+i*sizeof(float)*MAX_RANGE*LAG_SIZE*2, sizeof(float)*MAX_RANGE*LAG_SIZE*2);
        memcpy(xcfd, xcfd_arr+i*sizeof(float)*MAX_RANGE*LAG_SIZE*2, sizeof(float)*MAX_RANGE*LAG_SIZE*2);
