...
void *tmpbuf;
size_t tmpsze;
char progid[80]={"eclipsesound 2025/11/23"};
char progname[256];
int arg=0;
...
int main(int argc,char *argv[]) {
  ...
  int nintgs=12;      /* number of integration periods per scan */
  ...
  /*
    beam sequences for 24-beam MSI radars but only using 12 beams
   */
  int bmse[12] =
  ...
  fast_intt_sc = total_integration_usecs/1E6;
  fast_intt_us = total_integration_usecs - (fast_intt_sc*1e6);
  ...
  snd_nBeams_per_scan = snd_bms_tot*snd_freqs_tot;
